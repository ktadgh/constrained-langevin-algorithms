...
def reverse_gBAOAB_step(q_init,p_init,F, gs, h,M, gamma, k, kr,e):
    ...
    p = p + torch.bmm(L2, gamma*p.unsqueeze(-1)).squeeze(-1)*h + torch.bmm(L2, 2 * gamma * F(qp).unsqueeze(-1)).squeeze(-1)*h + torch.bmm(L2,math.sqrt(k*2*gamma)*R.unsqueeze(-1)).squeeze(-1)
...
p = torch.bmm(proj,torch.randn_like(q).unsqueeze(-1)).squeeze(-1).cuda()
